...
docker build -t 888780614544/graphsy .
# tag the image with the GitHub SHA
docker tag 888780614544/graphsy:latest 888780614544.dkr.ecr.us-east-2.amazonaws.com/graphsy:$GITHUB_SHA
...
zip -r graphsy-prod-deploy.zip Dockerrun.aws.json .ebextensions
# Upload zip file to s3 bucket
aws s3 cp graphsy-prod-deploy.zip s3://$EB_BUCKET/graphsy-prod-deploy.zip
# Create a new application version with new Dockerrun
aws elasticbeanstalk create-application-version --application-name g5 --version-label $GITHUB_SHA --source-bundle S3Bucket=$EB_BUCKET,S3Key=graphsy-prod-deploy.zip
